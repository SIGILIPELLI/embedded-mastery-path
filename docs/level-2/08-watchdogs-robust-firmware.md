...
**What a hardware watchdog physically is**: it's a free-running down-counter
timer clocked independently of the main CPU clock domain (often from its own
internal RC oscillator specifically so a main-clock fault can't also disable
the watchdog), wired directly to the chip's reset line. The counter
decrements every tick; if it ever reaches zero, dedicated logic asserts reset
with no CPU instruction involved at all — this is why a watchdog can recover
from a truly hung CPU (stuck in an infinite loop, or worse, executing
garbage after a stack corruption) that software-only recovery could never
detect from the inside. `esp_task_wdt_reset()` doesn't "check" anything —it
simply reloads that hardware counter back to its starting value, which is
exactly why calling it from the wrong place (a keepalive timer instead of the
struggling task) defeats the whole mechanism: the counter gets reloaded
regardless of whether the thing you actually care about monitoring made any
progress at all.

**How the Task WDT knows *which* task hung**: unlike a single global
hardware watchdog, `esp_task_wdt` is a software layer built on top of one
hardware timer — it maintains a list of subscribed task handles and, on each
underlying timer tick, checks whether every subscribed task has called
`esp_task_wdt_reset()` since the last check. When one hasn't, before letting
the underlying reset happen it deliberately triggers a **panic handler**
(`trigger_panic = true`) which walks each CPU core's current call stack
using the saved frame pointers/return addresses on that task's stack — the
same stack-frame mechanism from function calls generally — and prints a
symbolicated backtrace naming the exact task and the exact function it was
in when the timeout fired. This is why the "block" exercise (a mutex held by
one task, timing out a *different* task waiting on it) reports the *waiting*
task, not the guilty holder: the WDT only sees who failed to check in, and a
task blocked forever on `xSemaphoreTake` never gets the CPU back to call
`esp_task_wdt_reset()` at all — the actual culprit is inferred by you, from
context, not printed for you.

**Why brownout produces silent corruption rather than a clean stop**: below
its minimum specified voltage, a CMOS digital circuit doesn't simply "turn
off" — logic gates' threshold voltages shift, flip-flops can latch
metastable or wrong values, and a flash write in progress (which needs a
specific, higher programming voltage generated by an on-chip charge pump)
can leave cells in a partially-programmed state that reads back as neither
a clean `0` nor `1` reliably. The brownout detector is a dedicated analog
comparator continuously watching the supply rail against a reference voltage,
completely independent of the digital logic it's protecting — specifically so
it keeps working even as the rest of the chip's behavior becomes undefined,
and can assert reset *before* logic instability produces corrupted state,
rather than after.
